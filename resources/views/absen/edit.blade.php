    ...
    <div class="flex justify-center">
        <div class="w-full lg:w-250 xl:w-250 p-6 bg-white rounded shadow-md">
            <div class=" mb-4">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('absen.update', ['user_id' => $absen->user_id]) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                            ...
                            <div class="col-md-6">
                                <select id="jurusan_id" name="jurusan_id" class="form-control @error('jurusan_id') is-invalid @enderror" required>
                                    @foreach($jurusan as $jurusans)
                                        <option value="{{ $jurusans->id }}" {{ $jurusans->id == $absen->jurusan_id ? 'selected' : '' }}>{{ $jurusans->nama_jurusan }}</option>
                                    @endforeach
                                </select>
                                @error('jurusan_id')
                                    <span class="invalid-feedback" role="alert">
