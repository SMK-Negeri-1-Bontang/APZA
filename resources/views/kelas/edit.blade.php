    ...
    <form action="{{ route('kelas.update', $kelas->id) }}" method="POST" class="space-y-4">
        ...
        <div class="flex flex-col">
            ...
            <select name="nama_kelas" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                <option value="X" @selected($kelas->nama_kelas == 'X')">X</option>
                <option value="XI" @selected($kelas->nama_kelas == 'XI')">XI</option>
                <option value="XII" @selected($kelas->nama_kelas == 'XII')">XII</option>
            </select>
        </div>
        ...
        <div class="flex flex-col">
            ...
            <select name="jurusan_id" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                @foreach($jurusans as $jurusan)
                    <option value="{{ $jurusan->id }}" @selected($kelas->jurusan_id == $jurusan->id)>{{ $jurusan->nama_jurusan }}</option>
                @endforeach
            </select>
        </div>
